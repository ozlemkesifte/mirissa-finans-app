...
struct PropertyTests {
    ...
    @Test func yillikHedefAylikVeGunlukTutarli() {
        ...
        for tur in 0..<150 {
            ...
            for t in plan.targets {
                // Yıllık hedef ayların toplamı; ayda/günde yıl geneli ortalama
                ...
                #expect((t.enYogunAy?.siparis ?? 0) == (t.aylik.values.max() ?? 0), "tur \(tur)")
                if let y = t.enYogunAy {
                    let gun = Dates.daysInMonth(year: Dates.year(of: y.ay), month: Dates.monthNumber(of: y.ay))
                    #expect(y.gunluk == Int(ceil(Double(y.siparis) / Double(gun))), "tur \(tur)")
                }
